...
git apply --reverse --check ../../device/infinix/X6837/patches/0001-Change-haptics-activation-file-path.patch || REVERSE=$?
cd ../../
...
if [ -f "$TFILE" ];then
    ...
    cd bootable/recovery
    git apply ../../device/infinix/X6837/patches/0001-Change-haptics-activation-file-path.patch || RET=$?
    cd ../../
    if [ $RET -ne 0 ];then
